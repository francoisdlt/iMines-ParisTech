...
- (void)viewDidLoad
{
    ...
    NSString *url = [@"http://www.viedemineur.fr/imines.php?action=readcomm&id=" stringByAppendingFormat:@"%d",idVDM]; 
...
- (IBAction)segmentAction:(id)sender
{
    ...
    NSString *secretKey = @"8057284fbde0738117767f5deebfc816";
    ...
    switch (segmentedControl.selectedSegmentIndex) {
        case 0:
            NSLog(@"valide");
            // on like
            url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.viedemineur.fr/imines.php?action=like&key=%@&id=%d",secretKey,idVDM]];
        ...
        case 1:
            NSLog(@"bien fait");
            // on dislike
            url = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.viedemineur.fr/imines.php?action=dislike&key=%@&id=%d",secretKey,idVDM]];
            urlRequest = [[NSURLRequest alloc] initWithURL:url];
            connection = [[NSURLConnection alloc] initWithRequest:urlRequest delegate:self];
            break;
        default:
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if([connection.description rangeOfString:@"action=like"].location == NSNotFound && [connection.description rangeOfString:@"action=dislike"].location == NSNotFound){
